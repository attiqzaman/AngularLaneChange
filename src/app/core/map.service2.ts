...
export class MapService {
  ...
  initMap(): void {
	...
	const drawRouteListener = () => {
		this.calculateAndDisplayRoute(directionsService, directionsRenderer, map, startLatLng, endLatLng, false);
	};
	...
	const drawPointsListener = () => {
		this.calculateAndDisplayRoute(directionsService, directionsRenderer, map, startLatLng, endLatLng, true);
	};
	...
	const on1stBtnClickHandler = () => {
		startLatLng = latLng;
		startLatLng = new google.maps.LatLng(46.703403, -92.299917);
	...
	const on2ndBtnClickHandler = () => {
		endLatLng = latLng;
		endLatLng = new google.maps.LatLng(46.726056, -92.217565);
  ...
  interpolatePoints(pointsList: google.maps.LatLng[]): google.maps.LatLng[] {
	...
	for (var i = 0; i <= pointsList.length - 2; i++) {
		...
		let test = google.maps.geometry.spherical.computeDistanceBetween(
			pointsList[0], pointsList[pointsList.length - 1]
		);
